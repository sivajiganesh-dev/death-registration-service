...
public class DeathApplicationValidator {
    ...
    public List<DeathRegistrationApplication> validateApplicationUpdateRequest(DeathRegistrationRequest deathRegistrationRequest) {
        List<String> ids = deathRegistrationRequest.getDeathRegistrationApplications().stream().map(DeathRegistrationApplication::getId).collect(
            Collectors.toList());
        List<DeathRegistrationApplication> deathRegistrationApplications = repository.getApplications(DeathApplicationSearchCriteria.builder().ids(ids).build());
